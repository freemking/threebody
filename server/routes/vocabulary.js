...
router.get('/list', async (req, res) => {
    try {
        const [rows] = await queryWithRetry(
            'SELECT * FROM vocabulary_book ORDER BY added_time DESC'
        );
        // 转换字段名为驼峰
...
router.post('/add', async (req, res) => {
    try {
        ...
        const [existing] = await queryWithRetry(
            'SELECT * FROM vocabulary_book WHERE word = ?', [word]
        );

        if (existing.length > 0) {
            // 已存在，更新
            await queryWithRetry(
                `UPDATE vocabulary_book SET 
                    meaning = IF(? != '', ?, meaning),
            ...
            await queryWithRetry(
                `INSERT INTO vocabulary_book 
                    (word, meaning, phonetic, example, root_affix, grade, source, added_time, mastered, study_count)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 0)`,
                [word, meaning || '', phonetic || '', example || '', rootAffix || '', grade || '', source || 'wrongbook', now]
            );
        }
...
router.post('/add-batch', async (req, res) => {
    try {
        ...
        for (const item of words) {
            ...
            // 检查是否已存在
            const [existing] = await queryWithRetry(
                'SELECT * FROM vocabulary_book WHERE word = ?', [item.word]
            );
            ...
            if (existing.length > 0) {
                ...
                await queryWithRetry(
                    `INSERT INTO vocabulary_book 
                        (word, meaning, phonetic, example, root_affix, grade, source, added_time, mastered, study_count)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 0)`,
                    [item.word, item.meaning || '', item.phonetic || '', item.example || '', item.rootAffix || '', item.grade || '', item.source || 'wrongbook', now]
                );
                added++;
            }
...
router.post('/remove', async (req, res) => {
    try {
        ...
        await queryWithRetry('DELETE FROM vocabulary_book WHERE word = ?', [word]);
        res.json({ success: true });
    } catch (error) {
...
router.post('/mastered', async (req, res) => {
    try {
        ...
        await queryWithRetry(
            'UPDATE vocabulary_book SET mastered = ? WHERE word = ?',
            [mastered ? 1 : 0, word]
        );
...
router.post('/clear-mastered', async (req, res) => {
    try {
        await queryWithRetry('DELETE FROM vocabulary_book WHERE mastered = 1');
        res.json({ success: true });
    } catch (error) {
...
router.post('/clear-all', async (req, res) => {
    try {
        await queryWithRetry('DELETE FROM vocabulary_book');
        res.json({ success: true });
    } catch (error) {
...
router.get('/today', async (req, res) => {
    try {
        ...
        const [unmasteredWords] = await queryWithRetry(
            'SELECT * FROM vocabulary_book WHERE mastered = 0 ORDER BY RAND() LIMIT 5'
        );
        
        // 如果今天已经学了5个，返回已学习的单词
        if (studiedWords.length >= 5) {
            const [todayWords] = await queryWithRetry(
                'SELECT DISTINCT word, meaning, phonetic, example, root_affix, grade FROM vocabulary_book WHERE word IN (?)',
                [studiedWords]
            );
...
router.post('/study', async (req, res) => {
    try {
        ...
        // 更新词库的学习次数
        await queryWithRetry(
            'UPDATE vocabulary_book SET study_count = study_count + 1, last_study_time = ? WHERE word = ?',
            [now, word]
        );
        ...
        if (correct) {
            ...
            if (totalRecord.length > 0) {
                ...
                if (record.correct_count >= 3 && accuracy >= 0.8) {
                    await queryWithRetry(
                        'UPDATE vocabulary_book SET mastered = 1 WHERE word = ?',
                        [word]
                    );
                    await queryWithRetry(
                        'UPDATE vocabulary_total_record SET mastered = 1, mastered_time = NOW() WHERE word = ?',
...
router.get('/daily-record', async (req, res) => {
    try {
        ...
        const [rows] = await queryWithRetry(
            `SELECT dr.*, vb.meaning, vb.phonetic, vb.example 
             FROM vocabulary_daily_record dr 
             LEFT JOIN vocabulary_book vb ON dr.word = vb.word 
             WHERE dr.study_date = ? 
...
router.get('/total-record', async (req, res) => {
    try {
        const [rows] = await queryWithRetry(
            `SELECT tr.*, vb.meaning, vb.phonetic 
             FROM vocabulary_total_record tr 
             LEFT JOIN vocabulary_book vb ON tr.word = vb.word 
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const [totalWords] = await queryWithRetry(
            'SELECT COUNT(*) as count FROM vocabulary_book'
        );
        
        // 获取已掌握单词数
        const [masteredWords] = await queryWithRetry(
            'SELECT COUNT(*) as count FROM vocabulary_book WHERE mastered = 1'
        );
